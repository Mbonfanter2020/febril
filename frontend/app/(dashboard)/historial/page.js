...
export default function HistorialPage() {
  ...
  const handleEditObservation = (ev, id) => {
    ...
    const newVal = prompt(t('clinicalObservationPrompt'), current);
    if (newVal !== null) {
  ...
  function DetailPanel({ datos, probs, factores }) {
    ...
    return (
      <div style={{ padding: '1.25rem 1.5rem', background: 'var(--bg-secondary)', borderLeft: '4px solid var(--accent)' }}>
        ...
        {/* Probabilidades */}
        <Section title={t('probabilitiesSection')}>
          <Field label={t('mild')} value={p.leve != null ? `${p.leve}%` : null} color="var(--severity-low)" />
          <Field label={t('moderate')} value={p.moderada != null ? `${p.moderada}%` : null} color="var(--severity-mid)" />
          <Field label={t('severe')} value={p.severa != null ? `${p.severa}%` : null} color="var(--severity-high)" />
        </Section>

        {/* Datos generales */}
        <Section title={t('generalDataSection')} color="var(--text-muted)">
          <Field label={t('sex')} value={translateValue(d.sexo)} />
          <Field label={t('area')} value={translateValue(d.area)} />
          <Field label={t('daysOfFever')} value={d.tiempo_fiebre != null ? `${d.tiempo_fiebre} ${t('days')}` : null} />
          <Field label={t('vaccination')} value={translateValue(d.vacunacion)} />
        </Section>

        {/* Evaluación clínica */}
        <Section title={t('clinicalEvaluationSection')} color="#60a5fa">
          <Field label={t('nutritionalStatus')} value={translateValue(d.estado_nutricional)} />
          <Field label={t('glasgow')} value={d.glasgow != null ? d.glasgow : null} />
          <Field label={t('physicalFindingColumn')} value={translateValue(d.hallazgo_examen_fisico)} fullWidth={true} />
          <Field label={t('personalHistory')} value={translateValue(d.antecedentes_personales)} fullWidth={true} />
          <Field label={t('epidemiologicalContact')} value={translateValue(d.contacto_epidemiologico)} fullWidth={true} />
          <Field label={t('environmentalExposure')} value={translateValue(d.exposicion_ambiental)} />
        </Section>

        {/* Laboratorios */}
        <Section title={t('laboratories')} color="#34d399">
          <Field label={t('procalcitonin')} value={d.procalcitonina != null ? d.procalcitonina : null} />
          <Field label={t('leukocytes')} value={d.leukocytes != null ? d.leukocytes.toLocaleString() : null} />
          <Field label={t('crp')} value={d.pcr != null ? d.pcr : null} />
          <Field label={t('platelets')} value={d.plaquetas != null ? d.plaquetas.toLocaleString() : null} />
          <Field label={t('serumAlbumin')} value={d.albumina != null ? d.albumina : null} />
          <Field label={t('serumGlobulin')} value={d.globulina != null ? d.globulina : null} />
          <Field label={t('absoluteBandCells')} value={d.cayados_absolutos != null ? d.cayados_absolutos : null} />
        </Section>
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1>{t('patientHistory').split(' ')[0]} <span className="text-gradient">{t('patientHistory').split(' ').slice(1).join(' ')}</span></h1>
      ...
      <div className="card">
        ...
        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.75rem' }}>
          {t('showing')} {paginated.length} {t('of')} {filtered.length} {t('evaluations')}
        </div>
            ...
            <div className="table-container desktop-only">
              <table className="table">
                <thead>
                  <tr>
                    <th>{t('dateHeader')}</th>
                    <th>{t('ageGroupHeader')}</th>
                    <th>{t('genderHeader')}</th>
                    <th>{t('severityHeader')}</th>
                    <th>{t('confidenceHeader')}</th>
                    <th>{t('observationHeader')}</th>
                    ...
                    return (
                      <Fragment key={e.id}>
                        ...
                        >
                          ...
                          <td>{translateValue(datos.grupo_edad) || '—'}</td>
                          <td>{translateValue(datos.sexo) || '—'}</td>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginTop: '0.75rem', fontSize: '0.82rem' }}>
                        <div>
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.68rem', textTransform: 'uppercase' }}>{t('age')}</span>
                          <div style={{ fontWeight: 600 }}>{translateValue(datos.grupo_edad) || '—'}</div>
                        </div>
                        <div>
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.68rem', textTransform: 'uppercase' }}>{t('sex')}</span>
                          <div style={{ fontWeight: 600 }}>{translateValue(datos.sexo) || '—'}</div>
                        </div>
                        <div>
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.68rem', textTransform: 'uppercase' }}>{t('area')}</span>
                          <div style={{ fontWeight: 600 }}>{translateValue(datos.area) || '—'}</div>
                        </div>
                        <div>
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.68rem', textTransform: 'uppercase' }}>{t('daysOfFever')}</span>
                      ...
                      <div style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        >
                          ...
                          <span style={{ fontSize: '0.8rem', color: observaciones[e.id] ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                            {observaciones[e.id] || t('addClinicalObservation')}
                          </span>
                        </div>
                      </div>
                    )}
