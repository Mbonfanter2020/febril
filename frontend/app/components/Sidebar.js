...
const navItems = [
  { href: '/dashboard', icon: IconDashboard, labelKey: 'dashboard' },
  { href: '/evaluacion', icon: IconEvaluation, labelKey: 'patientEvaluation' },
  { href: '/rendimiento', icon: IconPerformance, labelKey: 'modelPerformance' },
  { href: '/historial', icon: IconHistory, labelKey: 'patientHistory' },
  { href: '/perfil', icon: IconUser, labelKey: 'user' },
];
  ...
  return (
    <>
          ...
          <div className="mobile-menu-content">
            ...
            >
              ...
              <div className="mobile-user-details">
                <div className="mobile-user-name">{profile?.nombre || t('user')}</div>
                <div className="mobile-user-role">{profile?.especialidad || t('researcher')}</div>
              </div>
            ...
            <div className="mobile-menu-actions">
              ...
              <button className="mobile-menu-item" onClick={() => { toggleLanguage(); setMobileMenuOpen(false); }}>
                <span className="mobile-icon-wrapper">
                  {language === 'es' ? '🇺🇸' : '🇪🇸'}
                </span>
      ...
      <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <div style={{ marginTop: 'auto', paddingTop: '0.75rem', borderTop: '1px solid var(--border)' }}>
            ...
            <button className="nav-item" onClick={toggleLanguage} style={{ cursor: 'pointer' }}>
              <span className="nav-icon">{language === 'es' ? '🇺🇸' : '🇪🇸'}</span>
              {language === 'es' ? 'English' : 'Español'}
            </button>
          </div>
        </nav>
        ...
        <div className="sidebar-footer">
          <div className="sidebar-user">
            ...
            <div className="sidebar-user-info">
              <div className="name">{profile?.nombre === 'Sin nombre' ? t('unnamed') : (profile?.nombre || t('user'))}</div>
              <div className="role">{profile?.especialidad || t('researcher')}</div>
            </div>
          </div>
          <button
            ...
            onClick={signOut}
          >
            <IconLogout style={{ width: '0.9em', height: '0.9em' }} /> {t('logout')}
          </button>
        </div>
      </aside>
      ...
      <nav className="bottom-nav">
        {[
          { icon: IconDashboard, labelKey: 'dashboard', href: '/dashboard' },
          { icon: IconEvaluation, labelKey: 'patientEvaluation', href: '/evaluacion' },
          { icon: IconPerformance, labelKey: 'modelPerformance', href: '/rendimiento' },
          { icon: IconHistory, labelKey: 'patientHistory', href: '/historial' },
        ].map((item) => {
