...
# Nombres de columnas exactos que espera el pipeline V2D
COLUMN_NAMES = [
    "Grupo edad años",
    "Sexo",
    "Area",
...
class MLService:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.modelo = None
            cls._instance.scaler = None
            cls._instance.ohe = None
            cls._instance.imputer_num = None
            cls._instance.imputer_cat = None
            cls._instance.cols_num = None
            cls._instance.cols_cat = None
            cls._instance.cols_escalar = None
            cls._instance.feature_names_post_ohe = None
            cls._instance.features_originales = None
            cls._instance.categorias_raras = None
    ...
    def load(self, pipeline_path: str, metadata_path: str, features_path: str):
        ...
        try:
            ...
            self.cols_num = pipeline_dict.get("cols_num", [])
            self.cols_cat = pipeline_dict.get("cols_cat", [])
            ...
            self.feature_names_post_ohe = self.feature_names["features_post_ohe"]
            self.features_originales = self.feature_names["features_originales"]
            self.categorias_raras = self.feature_names["categorias_raras"]
    ...
    def _group_rare_categories(self, data: dict) -> dict:
        ...
        field_to_col = {
            ...
            "nivel_triage_tep": "Nivel de Triage por el TEP",
        }
    ...
    def _build_dataframe(self, data: dict) -> pd.DataFrame:
        """Construye DataFrame con nombres exactos de columnas para el modelo V2D.

        El pipeline V2D incluye indicadores de missingness para albúmina,
        globulina, procalcitonina y PCR dentro de cols_num.
        """
        ...
        row = {
            ...
            "Nivel de Triage por el TEP": grouped.get("nivel_triage_tep", "Nivel III"),
            ...
            # Indicadores de missingness (esperados por el pipeline V2D)
            ...
            "Procalcitonina_ng_mL_missing": 1 if grouped.get("procalcitonina") is None else 0,
            "Proteina_C_reactiva_mg_Dl_missing": 1 if grouped.get("pcr") is None else 0,
        }
        ...
        all_cols = self.features_originales + [
            ...
            "Procalcitonina_ng_mL_missing",
            "Proteina_C_reactiva_mg_Dl_missing",
        ]
    ...
    def _apply_pipeline(self, df: pd.DataFrame) -> tuple:
        """
        Aplica el pipeline paso a paso (compatible con modelo V2D):
        ...
        # 4. Escalar numéricos (eliminado en V2D, no hay scaler)
        X_num_processed = X_num_imputed.values

        # 5. Concatenar respetando el orden de feature_names_post_ohe
        if X_missing_flags is not None:
            X_final = np.hstack([X_num_processed, X_missing_flags, X_cat_ohe])
        else:
            X_final = np.hstack([X_num_processed, X_cat_ohe])
    ...
    def _identify_factors(self, data: dict) -> list:
        ...
        triage = data.get("nivel_triage_tep")
        if triage in ["Nivel I", "Nivel II"]:
            factors.append(f"Alto riesgo por TEP ({triage})")
        ...
        triage = data.get("nivel_triage_tep", "Ninguno")
        if triage and triage not in ("Ninguno", ""):
            factors.append(f"Triage TEP: {triage}")
